...
const Menfant = ({ annonces, searchQuery }) => {
  ...
  useEffect(() => {
    ...
    const filteredByCategory = annonces.filter(annonce => annonce.category === 'mode-enfant');
    ...
    <> 
      <div className="min-h-screen bg-[#F4F6F9] p-6 relative">
        <div className="container mx-auto p-4">
          <h1 className="text-4xl font-extrabold text-center mb-8 text-[#333333]">MODE ENFANT</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredAnnonces.length > 0 ? (
              filteredAnnonces.map((annonce) => (
                <div key={annonce.id} className="bg-white p-2 cursor-pointer rounded-lg shadow-lg">
