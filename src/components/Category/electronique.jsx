...
const Electronique = ({ annonces, searchQuery }) => {
  ...
  useEffect(() => {
    ...
    const filteredByCategory = annonces.filter(annonce => annonce.category === 'electronique');
    ...
    <> 
      <div className="min-h-screen bg-[#F4F6F9] p-6 relative">
        <div className="container mx-auto p-4">
          <h1 className="text-4xl font-extrabold text-center mb-8 text-[#333333]">ELECTRONIQUE</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredAnnonces.length > 0 ? (
              filteredAnnonces.map((annonce) => (
                <div key={annonce.id} className="bg-white cursor-pointer p-2 rounded-lg shadow-lg max-w-xs">
